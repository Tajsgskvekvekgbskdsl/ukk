...
@section('page-title', 'Report Peminjaman')

@section('content')
<div class="card">
    <div class="card-header">
        <form method="GET" action="{{ route('admin.report.peminjaman') }}" class="row g-2 align-items-end">
            <div class="col-md-3 col-6">
                <input type="text" name="search" value="{{ $search }}" class="form-control form-control-sm" placeholder="Cari anggota / buku...">
            </div>
            <div class="col-md-3 col-6">
                <label class="form-label small mb-0">Pinjam dari</label>
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            <thead>
                <tr><th>No</th><th>Anggota</th><th>Buku</th><th>Tgl Pinjam</th><th>Status</th></tr>
            </thead>
            <tbody>
                @forelse($transaksis as $i => $t)
                    <tr>
                        ...
                        <td><span class="badge {{ $t->status === 'dipinjam' ? 'bg-primary' : 'bg-success' }}">{{ ucfirst($t->status) }}</span></td>
                    </tr>
                @empty
